...
def backtest(start_date, end_date, hold_days, strategy, strategy2, data, weight='average', benchmark=None, stop_loss=None, stop_profit=None):
    ...
    for sdate, edate in dates:
        ...
        #自加
        stocks2 = strategy2(data)
        
        # hold the stocks for hold_days day
        s1 = price[stocks.index & price.columns][sdate:edate].iloc[1:]
        #自加
        s2 = price[stocks2.index & price.columns][sdate:edate].iloc[1:]
        
        sma_120 = price[stocks.index & price.columns][sdate - datetime.timedelta(days=1440):edate].rolling(120).mean().reindex(s1.index)
        #自加
        sma2_120 = price[stocks2.index & price.columns][sdate - datetime.timedelta(days=1440):edate].rolling(120).mean().reindex(s2.index)

        #自己加，配息:
        div1 = dividend[stocks.index & dividend.columns][sdate:edate]
        div2 = dividend[stocks2.index & dividend.columns][sdate:edate]
        div3 = [div1,div2]
        ...
        #     end = 1
        # add nstock history
        nstock[0][sdate] = len(stocks)
        nstock[1][sdate] = len(stocks2)
    ...
    # plt.grid(linestyle='-.')
    # plt.show()
    # pd.Series(nstock[0]).plot.bar()
    # pd.Series(nstock[1]).plot.bar()
    # plt.ylabel('Number of stocks held')
    # plt.show()
    pd.DataFrame( {'strategy1': profit[0],'strategy2':profit[1] }).plot.bar()
    plt.ylabel('profit compare')
    plt.show()
